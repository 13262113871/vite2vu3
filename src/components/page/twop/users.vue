  ...
  <div class="out">
    ...
    <el-card shadow="hover">
      ...
      >
        <!-- 显示内容 -->
        <div>
          <el-form :model="quanUserinfo" label-width="80px">
            <el-form-item label="用户名">
              <el-input v-model="quanUserinfo.username" disabled></el-input>
            </el-form-item>
            <el-form-item label="用户名">
              <el-input v-model="quanUserinfo.role_name" disabled></el-input>
            </el-form-item>
            <el-form-item label="权限">
              <el-select v-model="rolesListId" placeholder="分配角色">
                <el-option
                  :label="item.roleName"
                  :value="item.id"
        ...
        <template #footer>
          <span class="dialog-footer">
            <el-button @click="quanVisible = false">取 消</el-button>
            <el-button type="primary" @click="quanChange">确 定</el-button>
          </span>
        </template>
      </el-dialog>
...
export default {
  setup() {
    ...
    // 修改用户权限
    const quanUser = (e) => {
      stateT.quanUserinfo = e;

      // 获取下拉菜单中展示的数据列表
      request({
        url: "/roles",
      }).then((res) => {
        stateT.rolesList = res.data;
      });
      stateT.quanVisible = true;
    };

    // 确认修改
    const quanChange = () => {
      if (!stateT.rolesListId) {
        return ElMessage.warning("请选择分配的角色");
      }
      request({
        url: `/users/${stateT.quanUserinfo.id}/role`,
        method: "put",
        data: { rid: stateT.rolesListId },
      }).then((res) => {
        if (res.meta.status !== 200) return ElMessage.error("更新权限失败");
        ElMessage.success(res.meta.msg);
        getList();
      });
      stateT.quanVisible = false;
    };

    // 管理权限修改页面
    const closeQuan = () => {
      (stateT.quanUserinfo = {}), (stateT.rolesListId = "");
    };
